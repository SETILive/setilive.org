...
# Where:                Fthr = T / t / M
#
  user_online_timeout = 5.minutes # Timer for user activity for online count in minutes
  max_archive_classifications = 19 # Archive retire threshold
  min_live_threshold = 3 # Minimum users for followup
  subjects_per_activity = 12
  number_of_beams = 2; # Default value. Posted to Frank before each session.
  user_sec_per_beam = 10 # Time per waterfall under live marking rules
  followup_window = followup_deadline_time - followup_lead_time
  RedisConnection.set( 'max_archive_classifications', max_archive_classifications )
  RedisConnection.set( 'min_live_threshold', min_live_threshold )
  RedisConnection.set( 'number_of_beams', number_of_beams )  
  live_threshold_factor = 
    1.0 * followup_window / user_sec_per_beam / subjects_per_activity 
  RedisConnection.set( 'live_threshold_factor', live_threshold_factor )
